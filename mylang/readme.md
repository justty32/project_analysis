...
其編譯器編譯一個檔案時，會將檔案以"行", "單詞"的方式，切成一個二維list。每一行都是一個list，用lisp的方式處理。
	若一行的開頭是tab(無論幾個tab都視為一個)，那這行的層級就深一個list。
	比如：
word1 word2 -> (word1 word2)
(word1 word2
	(word_3_1 word_3_2)
	(word_4_1
		(word_4_2_1 word_4_2_2)))

本質上這是腳本語言，可以被用C++寫的解釋器執行(基本上就是抄lua)，具體方式如下：
...
其優先順序為反斜線>註解>">縮排>'>小括號>大括號>中括號>單詞。
...
關於資料結構，有一個系統函數叫做make。其使用方法如下：
	...
	(make "std::list<int>" 1 2 3)，這會嘗試將後面的參數弄成對應的C++資料結構，然後返回。
其原理是這樣的：
